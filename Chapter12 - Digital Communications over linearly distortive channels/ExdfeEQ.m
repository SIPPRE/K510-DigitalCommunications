% Matlab Program <ExdfeEQ.m>
% This Matlab exercise <ExdfeEQ.m> performs simulation of
% decision feedback equalization under 16-QAM baseband transmission
...
% We can now plot the power spectral densities of the two signals xrcos and xchout
...
% or use mpath=[1 0 0 .45]; % time-domain multipath channel
...
for ii=1:9;
    Eb2Naz(ii)=2*ii;
    Q(ii)=3*0.5*erfc(sqrt((2*10^(Eb2Naz(ii)*0.1)/5)/2)); %Compute the Analytical BER
end
